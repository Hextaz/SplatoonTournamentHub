...
import { Lock, Plus, Search, Trash2, Maximize2, Users, Loader2 } from "lucide-react";
...
export function PlacementPhaseClient({
  ...
  const filteredTeams = unassignedTeams.filter(t => t.name.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const handleSaveSeeding = async () => {
    if (!confirm("Sauvegarder le placement pour cette phase ? Cela régénérera les matchs associés.")) return;
    setIsSaving(true);
    
    // Construct payload
    const participants = seeds
      .map((team, index) => team ? { team_id: team.id, seed: index + 1 } : null)
      .filter(t => t !== null);
    ...
    if (!team) {
      return (
        ...
        >
          ...
          <span className="text-slate-400 text-xs font-medium uppercase tracking-wider">Empty</span>
        </div>
      );
    }

    return (
      <div className={`flex items-center justify-between ${small ? 'p-1.5' : 'p-2.5'} border border-slate-200 shadow-sm bg-white rounded group`}>
        <div className="flex items-center gap-2 overflow-hidden">
          <div className="flex-shrink-0 w-5 text-center text-xs font-bold text-slate-400">(#{slotNum})</div>
          <span className="text-slate-800 text-sm font-bold truncate">{team.name}</span>
        </div>
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button 
            onClick={(e) => { e.stopPropagation(); handleRemoveFromSlot(slotNum - 1); }}
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 auto-rows-max">
        {groups.map((groupSeeds, idx) => (
          <div key={idx} className="bg-white rounded-lg border border-slate-200 shadow-sm shadow-slate-200/50 overflow-hidden">
  ...
  const renderRightBracket = () => {
    ...
    if (!pairs) {
      return <div className="text-slate-400 p-8 text-center bg-slate-900 rounded-xl border border-slate-800">Aucun format d'arbre disponible pour cette taille ({totalSlots}).</div>;
    }
    ...
    return (
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8 min-h-[400px] overflow-x-auto flex">
        <div className="flex flex-col justify-around gap-6 py-4 w-64 flex-shrink-0">
          <h3 className="text-center font-bold text-slate-400 text-xs mb-4 uppercase tracking-wider">Round 1</h3>
              ...
              <div key={idx} className="relative">
                ...
                <div className="bg-white border text-sm text-slate-700 border-slate-200 rounded shadow-sm relative z-10 flex flex-col overflow-hidden">
                  <div className="border-b border-slate-100 flex items-stretch h-10 w-full relative">
                     {(!seeds[seedA-1]) ? (
                       <button onClick={() => handleOpenModal(seedA)} className="flex-1 px-3 text-left hover:bg-slate-50 transition-colors flex items-center justify-between group h-full">
                         <span className="text-slate-400 font-medium text-xs">Empty (Seed {seedA})</span>
                         <Plus className="w-3.5 h-3.5 text-green-500 opacity-0 group-hover:opacity-100" strokeWidth={3} />
                       </button>
                     ) : (
                       <div className="flex-1 px-3 flex items-center justify-between bg-white group h-full">
                         <div className="flex items-center gap-2 truncate">
                           <span className="text-slate-400 text-xs font-bold">(#{seedA})</span>
                           <span className="font-bold truncate text-sm">{seeds[seedA-1].name}</span>
                         </div>
                         <button onClick={() => handleRemoveFromSlot(seedA - 1)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100">
                           <Trash2 className="w-3.5 h-3.5" />
                         </button>
                       </div>
                     )}
                     <div className="absolute right-2 top-1/2 -translate-y-1/2 opacity-20 pointer-events-none"><Lock className="w-3 h-3"/></div>
                  </div>
                  
                  <div className="flex items-stretch h-10 w-full relative">
                     {(!seeds[seedB-1]) ? (
                       <button onClick={() => handleOpenModal(seedB)} className="flex-1 px-3 text-left hover:bg-slate-50 transition-colors flex items-center justify-between group h-full">
                         <span className="text-slate-400 font-medium text-xs">Empty (Seed {seedB})</span>
                         <Plus className="w-3.5 h-3.5 text-green-500 opacity-0 group-hover:opacity-100" strokeWidth={3} />
                       </button>
                     ) : (
                       <div className="flex-1 px-3 flex items-center justify-between bg-white group h-full">
                         <div className="flex items-center gap-2 truncate">
                           <span className="text-slate-400 text-xs font-bold">(#{seedB})</span>
                           <span className="font-bold truncate text-sm">{seeds[seedB-1].name}</span>
                         </div>
                         <button onClick={() => handleRemoveFromSlot(seedB - 1)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100">
                           <Trash2 className="w-3.5 h-3.5" />
                         </button>
                       </div>
                     )}
        ...
        <div className="flex flex-col justify-around py-4 w-64 ml-12 opacity-50 relative pointer-events-none">
          <h3 className="text-center font-bold text-slate-400 text-xs mb-4 uppercase tracking-wider">Round 2</h3>
           {Array.from({length: pairs.length / 2}).map((_, i) => (
             <div key={i} className="bg-white border text-sm text-slate-400 border-slate-200 rounded shadow-sm relative z-10 flex flex-col h-[81px] overflow-hidden">
                <div className="border-b border-slate-100 flex-1 px-3 flex items-center bg-slate-50">TBD</div>
                <div className="flex-1 px-3 flex items-center bg-slate-50">TBD</div>
             </div>
           ))}
        </div>
      </div>
  ...
  return (
    <>
      <div className="grid grid-cols-12 gap-6 flex-1 min-h-0">
        ...
        {/* LEFT COLUMN: Roster */}
        <div className="col-span-12 lg:col-span-4 bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl flex flex-col h-[calc(100vh-12rem)]">
                ...
                >
                  ...
                  {team ? (
                    <>
                      ...
                      <button 
                        onClick={(e) => { e.stopPropagation(); handleRemoveFromSlot(index); }}
                    ...
                    <>
                       <div className="flex-1 flex items-center justify-between opacity-50 group">
                         <span className="text-slate-400 font-medium text-sm">Vide...</span>
          ...
          <div className="p-4 border-t border-slate-800 bg-slate-900/80 shrink-0 backdrop-blur flex flex-col gap-2">
            ...
            >
              {isSaving ? <Loader2 className="w-5 h-5 animate-spin" /> : "✅ Enregistrer le seeding"}
            </button>
          </div>
        </div>

        {/* RIGHT COLUMN: Visual Preview */}
        <div className="col-span-12 lg:col-span-8 bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl flex flex-col h-[calc(100vh-12rem)]">
          <div className="px-6 py-4 border-b border-slate-800 bg-slate-800/50 shrink-0 flex justify-between items-center">
            <h2 className="text-lg font-bold text-white flex items-center gap-2">
              <Maximize2 className="w-5 h-5 text-slate-400" />
              Aperçu <span className="opacity-50 font-normal">({isGroups ? "Groupes" : "Élimination directe"})</span>
            </h2>
          </div>
          
          {/* Use light background for Right Column content specifically (per Toornament styling) */}
          <div className="p-6 md:p-8 flex-1 overflow-y-auto bg-slate-100">
         ...
         <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden border border-slate-200 animate-in fade-in zoom-in-95 duration-200">
           ...
           <div className="px-6 py-2 shrink-0">
             <div className="flex items-center gap-2">
               <input type="checkbox" id="dispo" checked readOnly className="w-4 h-4 rounded text-blue-600 accent-blue-600" />
               <label htmlFor="dispo" className="text-sm font-semibold text-slate-700">Disponible ({unassignedTeams.length})</label>
             </div>
           </div>
           ...
           <div className="flex-1 overflow-y-auto bg-slate-50 border-t border-b border-slate-200 custom-scrollbar">
             <table className="min-w-full divide-y divide-slate-200">
               <thead className="bg-white sticky top-0 z-10 shadow-sm">
                 <tr>
                   <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-blue-500 uppercase tracking-wider w-12"></th>
                   <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-blue-500 uppercase tracking-wider">Nom</th>
                   <th scope="col" className="px-6 py-3 text-right text-xs font-semibold text-blue-500 uppercase tracking-wider">Date de création</th>
                   <th scope="col" className="px-6 py-3 text-center text-xs font-semibold text-blue-500 uppercase tracking-wider">Seed</th>
                 </tr>
               </thead>
               <tbody className="bg-white divide-y divide-slate-100">
                 {filteredTeams.length === 0 ? (
                   <tr>
                     <td colSpan={4} className="px-6 py-8 text-center text-slate-500">Aucun participant trouvé.</td>
                   </tr>
                 ) : (
                   filteredTeams.map((team) => (
                     ...
                     >
                       ...
                       <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500 text-right">
                         {new Date(team.created_at).toLocaleString("fr-FR", { dateStyle: "short", timeStyle: "medium" })}
                       </td>
